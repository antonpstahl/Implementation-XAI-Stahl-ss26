"""Tests for utils.eval (generation aware loading + judge prompt).
...
Sichert den Skalierungs-Eval-Pfad ab:
  ...
  * load_scale_records baut die erwarteten Spalten inkl. generation/tool_calls und
    reports or enforces completeness.
  * build_judge_prompt produces valid JSON with the NB 05 schema and embeds the
    tool transcript only for pipeline 06 (and only if a trace is present).
"""
...
def _write_generation(results_dir: Path, pipeline: str, xai: str, iid: int,
    ...
    rec = {
        ...
        "xai_model":   xai,
        "instance_id": iid,
        "explanation": f"Explanation {pipeline}/{xai}/{iid}/g{gen_idx}",
...
def test_load_scale_records_gen_aware(tmp_path):
    ...
    # Template: 2 models  x  2 instances  x  1 gen = 4 rows
    assert len(df[df.pipeline == "00"]) == 4
    # 04: 2  x  2  x  3 = 12 rows
...
def test_load_scale_records_reports_missing(tmp_path, capsys):
    # Only write one of the expected files  to  the rest are missing
...
def test_build_judge_prompt_valid_json_schema(tmp_path):
    ...
    row = {"pipeline": "04", "explanation": "The hour drives demand."}
    ...
    assert gt["feature_values_readable"]["year"] == "2011"      # yr=0
    assert gt["feature_values_readable"]["time"] == "08:00"
    assert "tool_call_trace" not in gt                          # not pipeline 06


def test_build_judge_prompt_embeds_trace_for_tooluse(tmp_path):
    _write_local_explanation(tmp_path, "xgb", 101)
    row = {"pipeline": "06", "explanation": "Tool Use explanation."}
